{% with item._source as record %}
    <li class="list__item--beeldenzoeker {{ extraClasses }}">
        <a href="{% block link %}{% url collection_object database=item._type object_id=record.priref %}?query_id={{ query_id }}&total={{ items.total }}&current={{ current|collection_query_offset:pagelen:page }}{% endblock %}"{% if link_target %} target="{{ link_target }}"{% endif %}>
            {% block item_image %}
