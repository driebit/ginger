{% if m.config.mod_zzc_adlib.url %}
    <label class="control-label">{_ Sync databases _}</label>
    {% with m.config.mod_zzc_adlib.databases.list as current %}
        {% for database in m.zzc_adlib.listdatabases %}
                ...
                {% with "adlib_database_" ++ database.database|first as input_id %}
                    <div class="checkbox">
                        <label for="{{ input_id }}">
                            ...
                            {% wire id=input_id postback={toggle_database database=database_name} delegate=`m_zzc_adlib` %}
                            {{ database_name }}
                        </label>
                    </div>
                {% endwith %}
