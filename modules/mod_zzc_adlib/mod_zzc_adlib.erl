...
-include_lib("include/zzc_adlib.hrl").
...
pull_database_updates(Database, Since, StartFrom, Context) when is_binary(Since) ->
    ...
    case Records of
        [] ->
            lager:info("mod_zzc_adlib: Pulled ~p records modified after ~s from database ~s", [Total, Since, Database]),
            ok;
        _ ->
...
observe_search_query(#search_query{search = {adlib, _Args}} = Query, Context) ->
    zzc_adlib_search:search(Query, Context);
...
pull_updates_when_needed(Pid, Frequency, Context) ->
    case z_convert:to_integer(m_config:get_value(mod_zzc_adlib, poll_frequency, Context)) of
        Frequency ->
