    ...
    {% if id %}
        ...
        {% else %}
            ...
            {% if id.o.hasbanner[1].depiction.width > 500 %}
                <div class="masthead do_parallax {{ extraClasses }}" style="background-image: url({% image_url id.o.hasbanner[1].depiction.id mediaclass='masthead' crop %}); background-size: cover;"></div>
            {% elif id.o.header[1].depiction.width > 500 %}
                <div class="masthead do_parallax {{ extraClasses }}" style="background-image: url({% image_url id.o.header[1].depiction.id mediaclass='masthead' crop %}); background-size: cover;"></div>
            {% elif id.depiction.width > 500 and not (id.category.is_a.person or id.category.is_a.media) %}
                <div class="masthead do_parallax {{ extraClasses }}" style="background-image: url({% image_url id.depiction.id mediaclass='masthead' crop %}); background-size: cover;"></div>
            {% else %}
