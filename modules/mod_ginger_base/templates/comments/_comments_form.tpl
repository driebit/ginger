{% with m.acl.user as user_id %}
    {% if user_id or m.config.mod_comment.anonymous.value|default_if_undefined:1 %}
        ...
        {% else %}
            {% wire id="comments-form" type="submit" postback={newcomment id=id comment_template="comment/comment.tpl" action={update target="comments-header" template="comments/comments-header.tpl" id=id} action={update target="comments-header" template="comments/comments-header.tpl" id=id} } delegate="mod_comment" %}
        {% endif %}
