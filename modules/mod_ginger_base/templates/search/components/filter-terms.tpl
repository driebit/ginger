...
as
    ...
    options_template
%}
    <div class="search__filters__section is-open do_search_cmp_filter_terms  {{ load_more|if:"search__filters__section--load_more":"" }} " data-items-loaded="{{ items_loaded }}" data-property="{{ property }}" data-dynamic="{{ dynamic }}" data-sort-by-count="{{ sort_by_count }}" data-update-event="{{ #search_filter_update}}">
    ...
    {% wire name=#search_filter_update action={update target=#search_filter template=options_template dynamic=dynamic load_more=load_more length=length} %}
